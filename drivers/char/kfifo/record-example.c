...
static int __init testfunc(void)
{
	...
	printk(KERN_INFO "[fifo_rec]record fifo test start\n");

	kfifo_in(&test, &hello, sizeof(hello));

	/* show the size of the next record in the fifo */
	printk(KERN_INFO "[fifo_rec]fifo peek len: %u\n", kfifo_peek_len(&test));
	printk(KERN_INFO "[fifo_rec]fifo len: %u\n", kfifo_len(&test));
	printk(KERN_INFO "[fifo_rec]=================\n");
	...
	printk(KERN_INFO "[fifo_rec]skip 1st element\n");
	kfifo_skip(&test);
	...
	while (!kfifo_is_empty(&test)) {
		...
		buf[ret] = '\0';
		printk(KERN_INFO "[fifo_rec]item = %.*s\n", ret, buf);
		printk(KERN_INFO "[fifo_rec]fifo peek len: %u\n", kfifo_peek_len(&test));
	...
	printk(KERN_INFO "[fifo_rec]test passed\n");
