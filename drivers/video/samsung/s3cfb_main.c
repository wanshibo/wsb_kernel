...
static int s3cfb_probe(struct platform_device *pdev)
{
	...
	int i = 0;
	#if 1	//vincent modify
	lcd_reg_write(0x10010210,0x3); //syscon output path
	lcd_reg_write(0x1003c234,(lcd_reg_read(0x1003c234)&(~0xf))| 0x6); //clk_src_lcd0 = mpll
	lcd_reg_write(0x1003c534,(lcd_reg_read(0x1003c534)&(~0xf))| 0xB); //clk_src_lcd0 = mpll/12	

	lcd_reg_write(GPM2CON,(lcd_reg_read(GPM2CON)&(~(0xf << (4*3))))|(0x1 << (4*3)));
	lcd_reg_write(GPM2DAT,lcd_reg_read(GPM2DAT)&(~(0x1 << (3))));
	lcd_reg_write(GPM2CON,(lcd_reg_read(GPM2CON)&(~(0xf << (4*3))))|(0x1 << (4*3)));
	lcd_reg_write(GPM2DAT,lcd_reg_read(GPM2DAT)&(~(0x1 << (3))));
	lcd_reg_write(GPC1CON,(lcd_reg_read(GPC1CON)&(~(0xf << 8)))|(0x1 << 8));

	/*---->GPC0[1]:VLED_ON output high -->On 12V---------------*/
	lcd_reg_write(GPC0CON,(lcd_reg_read(GPC0CON)&(~(0xf << 4)))|(0x1 << 4));
	lcd_reg_write(GPC0DAT,(lcd_reg_read(GPC0DAT)&(~(0x1 << 1)))|(0x1 << 1));
	lcd_reg_write(GPC1CON,(lcd_reg_read(GPC1CON)&(~(0xf << 4)))|(0x1 << 4));
	lcd_reg_write(GPC1DAT,(lcd_reg_read(GPC1DAT)&(~(0x1 << 1)))|(0x1 << 1));

	/*---------GPIO CONTROL---------*/
	lcd_reg_write(GPF0CON, (lcd_reg_read(GPF0CON) & (~(0xFF<<16)))|0x22002222);	//set GPF0 as LVD_HSYNC,VSYNC,VCLK,VDEN,VD[3:0]
	lcd_reg_write(GPF1CON, (lcd_reg_read(GPF1CON) & (~(0xFF<<16)))|0x22002222);	//set GPF1 as VD[11:4]
	lcd_reg_write(GPF2CON, (lcd_reg_read(GPF2CON) & (~(0xFF<<16)))|0x22002222);	//set GPF2 as VD[19:12]
	lcd_reg_write(GPF3CON, (lcd_reg_read(GPF3CON) & (~(0xFF<<16)))|0x22002222);	//set GPF3 as VD[23:20]
	#endif
