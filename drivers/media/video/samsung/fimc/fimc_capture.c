...
retry:
	...
	ret = v4l2_subdev_call(cam->sd, core, init, ctrl->cam->sensor_index);	//vincent
...
int fimc_enum_input(struct file *file, void *fh, struct v4l2_input *inp)
{
	...
	mutex_unlock(&ctrl->v4l2_lock);
	//printk("wsbcamera: inp->index = %dinp-name = %s\n", inp->index, inp->name);
	return 0;
}
   ...
   // i=3;//gaopeng
	...
	if (i != fimc->active_camera) {
		...
		if ((ctrl->cam->id != CAMERA_WB) && (ctrl->cam->id !=
			...
			ctrl->cam->sensor_index=i;	//vincent
...
int fimc_s_fmt_vid_private(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	if (ctrl->cam->sd) {
		...
		printk("[ctrl->cam->sd]fimc_s_fmt_vid_private %d %d %s\n",pix->width,pix->height,ctrl->cam->sd->name);
		if (ctrl->cap)
			printk("no use isp\n");
		else{
			ctrl->cap = cap;
		...
		printk("[ctrl->cam->sd]fimc_s_fmt_vid_private width=%d height=%d depth=%d field=%d\n",mbus_fmt->width,mbus_fmt->height,depth,mbus_fmt->field);
		if (depth == 0) {
		...
		if (fimc_cam_use) {
			ret = v4l2_subdev_call(ctrl->cam->sd, video,
					       s_mbus_fmt, mbus_fmt);
			printk("[ctrl->cam->sd]fimc_s_fmt_vid_private ret=%d\n",ret);
			if (ret) {
...
int fimc_s_fmt_vid_capture(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	is_ctrl.value = 0;
	fimc_dbg("%s\n", __func__);
	//printk("fimc_s_fmt_vid_capture-------- %d %d\n",mbus_fmt->width,mbus_fmt->height);
	...
	*/
	if (!cap) {
		...
		printk("fimc_s_fmt_vid_capture--------11111111111\n");
		//memset(cap, 0, sizeof(*cap));
	}
	//printk("fimc_s_fmt_vid_capture--------222222222222 %d %d\n",mbus_fmt->width,mbus_fmt->height);
	...
	printk("fimc_s_fmt_vid_capture-------- %d\n",cap->fmt.field);
	if(cap->fmt.field==2){
		printk("###################################33\n");
...
int fimc_reqbufs_capture_mmap(void *fh, struct v4l2_requestbuffers *b)
{
	...
	printk("fimc_reqbufs_capture_mmap-------- %d\n",cap->fmt.field);
	...
	if ((cap->nr_bufs >= 0) && (cap->nr_bufs < FIMC_CAPBUFS)) {
		fimc_err("%s : remained previous buffer count is %d\n", __func__,
				cap->nr_bufs);
		for (i = 0; i < cap->nr_bufs; i++) {
	...
	case V4L2_PIX_FMT_NV61:		/* fall through */
		fimc_err("%s : 2plane for NV21 w %d h %d\n", __func__,
				cap->fmt.width, cap->fmt.height);
	...
	case V4L2_PIX_FMT_YUV420:
	case V4L2_PIX_FMT_YVU420:
		fimc_err("%s : 3plane %d %d\n", __func__,cap->fmt.width,cap->fmt.height);
		ret = fimc_alloc_buffers(ctrl, 3,
			cap->fmt.width * cap->fmt.height, 0, bpp, cap->pktdata_enable, cap->pktdata_size);
		break;

	case V4L2_PIX_FMT_JPEG:
		fimc_err("%s : JPEG 1plane\n", __func__);
		size = fimc_camera_get_jpeg_memsize(ctrl);
		fimc_err("%s : JPEG 1plane size = %x\n", __func__, size);
...
int fimc_test(struct file *file,void *fh)
{
	//struct fimc_control *ctrl = fh;
	struct fimc_control *ctrl = ((struct fimc_prv_data *)fh)->ctrl;
	struct fimc_capinfo *cap = ctrl->cap;
...
int fimc_streamon_capture(void *fh)
{
	...
	struct s3c_platform_fimc *pdata = to_fimc_plat(ctrl->dev);
	printk("############################ fimc_streamon_capture %d##############################\n",cap->fmt.priv);
	printk("fimc_streamon_capture-------- %d\n",cap->fmt.field);
	...
	/*if (cap->fmt.field==2) {
		...
		if (ret < 0) {
			...
			if (cam_frmsize.discrete.width > 0
				...
				printk("enum_framesizes width = %d,height = %d\n", cam->width,
					cam->height);
			}
		}
	}*/
