...
#define RPTVP5150_DRIVER_NAME	"tvp5150a"
#define SDCARD_FW
...
static int __i2c_read(struct v4l2_subdev *sd, unsigned char *reg, unsigned char *buf,int len)
{
...
static void check_video_status(struct v4l2_subdev * sd)
{
	char reg_84h, reg_85h,reg_88h, reg_89h, reg_8ch;
	char reg = 0x84;
	reg_84h=0;
	reg_85h=0;
	reg_88h=0;
	...
	reg_84h = i2c_smbus_read_byte_data(gclient, 0x84);
	
	reg_85h = i2c_smbus_read_byte_data(gclient, 0x85);
	
	reg_88h = i2c_smbus_read_byte_data(gclient, 0x88);
	mdelay(2);
	reg_89h = i2c_smbus_read_byte_data(gclient, 0x89);
	mdelay(2);
	reg_8ch = i2c_smbus_read_byte_data(gclient, 0x8c);
	mdelay(2);
	printk("tvp5150am1 per line = %d\n", ((reg_84h &0x03)<<8)| reg_85h);
	printk("tvp5150am1 reg88h = %d\t reg89h = %d\t reg8ch = %d\n", reg_88h, reg_89h, reg_8ch);
	printk("wait ...\n");
	mdelay(5);
...
static int rptvp5150_init(struct v4l2_subdev *sd, u32 val)
{
	...
	int i;
	int m;
	//unsigned char tvp5150am1_patch_code[5187][2];
	...
	//reset tvp5150
	gpio_direction_output(EXYNOS4212_GPV0(0), 0);
	mdelay(5);
	gpio_direction_output(EXYNOS4212_GPV0(0), 1);
	mdelay(20);
