...
#define SI4702RST		S5PV210_GPB(6)
...
static int fm_seekth_setting(struct si4702_data *si4702, unsigned int seekth)
{
	...
	regval->reg.seekth = min(seekth, SI4702_SEEKTH_MAX);
...
static int fm_volume_setting(struct si4702_data *si4702, unsigned int volume)
{
	...
	regval->reg.volume = min(volume, SI4702_VOLUME_MAX);
	if(si4702_write_regs(si4702->client, regval, 8))
		return -1;
...
static int si4702_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
