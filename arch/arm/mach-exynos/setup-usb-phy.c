...
#define GPIO_HUB_RESET EXYNOS4212_GPM3(2)
#define GPIO_HUB_CONNECT EXYNOS4212_GPM3(3)
#define BOARD_HUB_RESET EXYNOS4212_GPM4(1)
...
void usb_hub_gpio_init(void)
{
	...
	gpio_request(GPIO_HUB_INT, "HUB_INT");
	gpio_direction_output(GPIO_HUB_INT, 1);
	s3c_gpio_setpull(GPIO_HUB_INT, S3C_GPIO_PULL_NONE);

	// hub2 reset
	printk("vincent hub2 reset +++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	gpio_request(BOARD_HUB_RESET, "HUB_RESET");
	gpio_direction_output(BOARD_HUB_RESET, 1);
	udelay(10);
	gpio_direction_output(BOARD_HUB_RESET, 0);	
}
...
int s5p_usb_phy_init(struct platform_device *pdev, int type)
{
	...
	usb_hub_gpio_init(); //raymanfeng for hsic hub
