...
static const u16 wm8978_reg[WM8978_CACHEREGNUM] = {
	...
	0x0005,0x0001,				 /* 0x38...0x3b */ 
        ...
        0x00ff, 0x012c, 0x012c, 0x002c, /* 0x10...0x13 */
        0x002c, 0x002c, 0x002c, 0x0000, /* 0x14...0x17 */
        0x0032, 0x0000, 0x0000, 0x0000, /* 0x18...0x1b */
        0x0000, 0x0000, 0x0000, 0x0000, /* 0x1c...0x1f */
        0x01b8, 0x000b, 0x0032, 0x000a, /* 0x20...0x23 */
        0x0008, 0x000c, 0x0093, 0x00e9, /* 0x24...0x27 */
        0x0000, 0x0000, 0x0000, 0x0000, /* 0x28...0x2b */
        0x0033, 0x01bf, 0x01bf, 0x0100, /* 0x2c...0x2f */
        0x0100, 0x0002, 0x01e1, 0x01e1, /* 0x30...0x33 */
...
extern int earphone_inset_flag; 
...
static int wm8978_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute){
		...
		gpio_direction_output(EXYNOS4_GPL2(4),1);
			...
			snd_soc_update_bits(codec, 54, 0x40, 0x0);
			snd_soc_update_bits(codec, 55, 0x40, 0x0);
...
static int wm8978_startup(struct snd_pcm_substream *stream,struct snd_soc_dai *dai)
{
	printk("%s",__FUNCTION__);
/*	if(!earphone_inset_flag)
		gpio_direction_output(EXYNOS4212_GPM1(4),1);//spk on
	else		
		gpio_direction_output(EXYNOS4212_GPM1(4),0);//spk on
		*/
		return 0;
}
...
static int wm8978_resume(struct snd_soc_codec *codec)
{
	...
	printk("%s,earphone_inset_flag:%d",__FUNCTION__,earphone_inset_flag);
	suspend_flag=0;
